...
# CHECK IF FILE EXIST
def yahoo_path():
    path = 'Result/Yahoo_Validator/'
    if os.path.exists(path) and os.path.exists(f'{path}Yahoo_Invalid.txt') :
           os.remove(f'{path}Yahoo_Invalid.txt')
           os.remove(f'{path}Yahoo_Valid.txt')
           return
    else:
...
def valid(count,pathx,rm):
    ...
    browser.open('https://login.yahoo.com/', timeout=5)
    ...
    if count != -1  :
        if 'recaptcha?' in tam :
            with open('Result/Yahoo_Validator/Yahoo_Valid.txt','a') as gmal:
                gmal.write(tr[count])
                pass
            ...
            with open('Result/Yahoo_Validator/Yahoo_Invalid.txt','a') as gmail:
